...
current Git `HEAD`, each release step pass/fail result, the per-step evidence paths, and whether
hardware scope or real actuation is active. It also reports the modification time and age of the
latest release, audit, index, and repeatability evidence so headless operators can judge when a
fresh release-candidate run is needed. Use
