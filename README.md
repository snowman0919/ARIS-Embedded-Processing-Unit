...
`$ARIS_LOGS/readiness/latest_headless_release_candidate.json`. It also runs the non-actuating
operational readiness audit and writes `$ARIS_LOGS/readiness/latest_headless_status.json` after
refreshing the final evidence index, so the release bundle preserves the exact status summary used
for handoff.
