...
aris_compose run --rm aris-ros2-dev bash -lc '
  ...
  launch_log=/tmp/aris_v2a_drift_recovery_launch.log
...
rclpy.init()
node = rclpy.create_node("aris_v2a_drift_recovery_smoke")
...
with open("/tmp/aris_v2a_drift_recovery_metrics.txt", "w") as handle:
    handle.write(
        f"wheel_samples={len(wheel)} truth_samples={len(truth)} filtered_samples={len(filtered)} "
        f"matched_samples={len(matched)} max_x={max_x:.3f} "
...
with open("/tmp/aris_v2a_drift_recovery_status.txt", "w") as handle:
    handle.write(status + "\n" + reason + "\n")
PY
