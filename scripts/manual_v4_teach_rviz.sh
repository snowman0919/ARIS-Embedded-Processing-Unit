...
printf '\nARIS V4 visual teach\n'
printf '  route file: %s\n\n' "$route_host"
printf 'Open another terminal and run:\n'
printf '  cd %s && nix develop -c just teleop-key\n\n' "$ARIS_WS"
printf 'Drive while RViz shows Localization Pose and Recorded Teach Path.\n'
printf 'When done, press Ctrl-C in this teach terminal.\n\n'
...
aris_compose --profile gui run --rm aris-ros2-dev bash -lc "
  ...
  bringup_pid=\$!
  ros2 launch aris_planning v4_rviz.launch.py >/tmp/aris_manual_v4_teach_rviz.log 2>&1 &
  rviz_pid=\$!
  cleanup() {
